...
// AddFileToZip: eg. zipfn="c:\\archive.zip", zefn="c:\\docs\\file.txt",
// zename="file.txt" If the zipfile already contained something called zename
// (case-insensitive), it is removed.  These two functions are defined below.

int main() {
  printf(
      "This program shows how to modify an existing zipfile -- add to it, "
      "remove from it.\n");
  ...
  printf(
      "Creating '\\z\\modify.zip' with zna.txt, znb.txt, znc.txt and a "
      "folder...\n");
...
// zip) If zefn is empty, we just delete zename from the zip archive. The way it
...
ZRESULT AddFileToZip(const TCHAR *zipfn, const TCHAR *zename,
                     const TCHAR *zefn) {
  if (GetFileAttributes(zipfn) == INVALID_FILE_ATTRIBUTES ||
      (zefn != 0 && GetFileAttributes(zefn) == INVALID_FILE_ATTRIBUTES))
    return ZR_NOFILE;

  // Expected size of the new zip will be the size of the old zip plus the size
  // of the new file
  HANDLE hf =
      CreateFile(zipfn, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
  if (hf == INVALID_HANDLE_VALUE) return ZR_NOFILE;
  DWORD size = GetFileSize(hf, 0);
  CloseHandle(hf);
  if (zefn != 0) {
    hf =
        CreateFile(zefn, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
    if (hf == INVALID_HANDLE_VALUE) return ZR_NOFILE;
  ...
  hf = CreateFile(zipfn, GENERIC_WRITE, 0, 0, CREATE_ALWAYS,
