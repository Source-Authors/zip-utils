...
DWORD WINAPI Ex3ThreadFunc(void *dat) {
  ...
  if (!CloseHandle(hwrite)) {
    ...
    msg("* Failed to close pipe write end");
  }
...
struct ReleaseDcDeleter {
  void operator()(HDC__ *dc) const noexcept {
    if (!ReleaseDC(0, dc)) {
      DWORD rc = ::GetLastError();
      msg("* Failed to release device context");
      return exit(rc);
    }
  }
};
...
struct CompatibleDcDeleter {
  void operator()(HDC__ *dc) const noexcept {
    if (!DeleteDC(dc)) {
      DWORD rc = ::GetLastError();
      msg("* Failed to delete device context");
      return exit(rc);
    }
  }
};
...
BOOL CreateFiles() {
...

  {
    ...
    bitmap_ptr hbm{CreateDIBSection(sdc.get(), &bi, DIB_RGB_COLORS,
                                    (void **)&bits, NULL, 0)};
    if (!hbm) msg("Failed to create bitmap");
...

    {
      ...
      handle_ptr<msg> hf{CreateFile("\\simple.bmp", GENERIC_WRITE, 0, NULL,
                                    CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,
                                    NULL)};
      if (!hf) msg("* Failed to create '\\simple.bmp'");
      ...
      if (rc) {
        msg("Created '\\simple.bmp'");
      } else {
        msg("* Failed to write '\\simple.bmp'");
      }
    }

    {
      ...
      handle_ptr<msg> hf{CreateFile("\\simple.txt", GENERIC_WRITE, 0, NULL,
                                    CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,
                                    NULL)};
      if (!hf) msg("* Failed to create '\\simple.txt'");

      DWORD writ;
      BOOL rc = WriteFile(hf.get(), src, (DWORD)strlen(src), &writ, NULL);

      if (rc)
        msg("Created '\\simple.txt'");
      else
        msg("* Failed to write '\\simple.txt'");
    }
  }
...
int main() {
  ...
  if (!crc) msg("* Failed to create files");

  {
    // EXAMPLE 1 - create a zipfile from existing files
    zip_ptr hz{CreateZip("\\simple1.zip", 0)};
    if (!hz) msg("* Failed to create \\simple1.zip");

    ZRESULT rc = ZipAdd(hz.get(), "znsimple.bmp", "\\simple.bmp");
    if (rc != ZR_OK) msg("* Failed to add simple.bmp to \\simple1.zip");

    rc = ZipAdd(hz.get(), "znsimple.txt", "\\simple.txt");
    if (rc != ZR_OK) msg("* Failed to add simple.txt to \\simple1.zip");

    if (rc == ZR_OK) msg("Created '\\simple1.zip'");
  }

  {
    // EXAMPLE 2 - unzip it with the names suggested in the zip
    zip_ptr hz{OpenZip("\\simple1.zip", 0)};
    ZRESULT rc = SetUnzipBaseDir(hz.get(), "\\");
    if (rc != ZR_OK) {
      msg("* Failed to set unzip base dir for \\simple1.zip");
    }

    ZIPENTRY ze;
    rc = GetZipItem(hz.get(), -1, &ze);
    if (rc != ZR_OK) {
      msg("* Failed to get items count for zip \\simple1.zip");
    }
    ...
    for (int zi = 0; zi < numitems; zi++) {
      ...
      if (rc != ZR_OK) {
        msg("* Failed to get N item for zip \\simple1.zip");
      }

      rc = UnzipItem(hz.get(), zi, ze.name);
      if (rc != ZR_OK) {
        msg("* Failed to unzip N item for zip \\simple1.zip");
      }
    }

    if (rc == ZR_OK) {
      msg("Unzipped 'znsimple.bmp' and 'znsimple.txt' from 'simple1.zip'");
    }
  }

  {
    ...
    if (!hz) msg("* Failed to create ppassword protected zip");

    // adding a conventional file...
    ZRESULT rc = ZipAdd(hz.get(), "znsimple.txt", "\\simple.txt");
    if (rc != ZR_OK) {
      msg("* Failed to add simple.txt to password protected zip");
    }
    ...
    rc = ZipAdd(hz.get(), "simple.dat", buf, 1000);
    if (rc != ZR_OK) {
      msg("* Failed to add simple.dat to password protected zip");
    }
    ...
    if (CreatePipe(&hread, &hwrite, NULL, 0)) {
      ...
      if (hthread) {
        ...
        rc = ZipAddHandle(hz.get(), "simple3.dat", hread.get(), 1000);
        if (rc != ZR_OK) {
          msg("* Failed to add simple3.dat from pipe to password protected "
              "zip");
        }
        ...
        // Close hwrite.
        if (!CloseHandle(hwrite)) msg("* Failed to close pipe write end");
    ...
    if (rc != ZR_OK) msg("* Failed to get zip memory");

    handle_ptr<msg> hfz{CreateFile("\\simple3 - pwd is 'password'.zip",
                                   GENERIC_WRITE, 0, 0, CREATE_ALWAYS,
                                   FILE_ATTRIBUTE_NORMAL, 0)};
    if (!hfz) {
      msg("* Failed to create zip '\\simple3 - pwd is 'password'.zip'");
    }
    ...
    if (wrc)
      msg("Created '\\simple3 - pwd is 'password'.zip' via pagefile");
    else
      msg("* Failed to write 'zip \\simple3 - pwd is 'password'.zip'");
  }

  {
    ...
    if (!hrsrc) msg("* Failed to find 1 resource");

    HANDLE hglob = LoadResource(hInstance, hrsrc);
    if (!hglob) msg("* Failed to load 1 resource");

    void *zipbuf = LockResource(hglob);
    if (!zipbuf) msg("* Failed to lock 1 resource");

    unsigned int ziplen = SizeofResource(hInstance, hrsrc);
    if (!ziplen) msg("* Failed to get size of 1 resource");

    zip_ptr hz{OpenZip(zipbuf, ziplen, 0)};
    if (!hz) msg("* Failed to create zip in memory");

    ZRESULT rc = SetUnzipBaseDir(hz.get(), "\\");
    if (rc != ZR_OK) msg("* Failed to set unzip base dir to '\\\\'");

    int i;
    ZIPENTRY ze;
    rc = FindZipItem(hz.get(), "simple.jpg", true, &i, &ze);
    if (rc != ZR_OK) msg("* Failed to find 'simple.jpg' in zip");

    //   - unzip to a file -
    rc = UnzipItem(hz.get(), i, ze.name);
    if (rc != ZR_OK) msg("* Failed to unzip 'simple.jpg' to file");

    {
      ...
      if (rc != ZR_OK) {
        msg("* Failed to unzip 'simple.jpg' to heap memory buffer");
      }
    }
    ...
    while (rc == ZR_MORE) {
      ...
      if (rc != ZR_OK && rc != ZR_MORE) {
        msg("* Failed to unzip 'simple.jpg' to stack memory buffer");
      }
    ...
    if (rc == ZR_OK)
      ...
      msg("Unzipped 'simple.jpg' to stack memory buffer from resource zipfile");
    else
      msg("* Failed to unzip 'simple.jpg' to stack memory buffer from "
          "resource zipfile");
  }

  if (IsFileExist("\\simple.txt")) {
    if (!DeleteFile("\\simple.txt")) {
      msg("* Failed to delete file '\\simple.txt'");
    } else {
      msg("Deleted file '\\simple.txt'");
    }
  }

  if (IsFileExist("\\simple.bmp")) {
    if (!DeleteFile("\\simple.bmp")) {
      msg("* Failed to delete file '\\simple.bmp'");
    } else {
      msg("Deleted file '\\simple.bmp'");
    }
  }
