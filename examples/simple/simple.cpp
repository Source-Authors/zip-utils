...
int main() {
  ...
  HZIP hz = CreateZip(_T("\\simple1.zip"), 0);
  ...
  CloseZip(hz);
  _tprintf(
      _T("Unzipped 'znsimple.bmp' and 'znsimple.txt' from 'simple1.zip'\n"));

  // EXAMPLE 3 - create an auto-allocated pagefile-based zip file from various
  // sources the second argument says how much address space to reserve for it.
  // We can afford to be generous: no address-space is allocated unless it's
  // actually needed.
  ...
  char buf[1000];
  for (int zj = 0; zj < 1000; zj++) buf[zj] = (char)(zj & 0x7F);
  ...
  if (CreatePipe(&hread, &hwrite, NULL, 0)) {
    ...
    HANDLE hthread = CreateThread(0, 0, Ex3ThreadFunc, hwrite, 0, 0);
    if (hthread) {
      // the '1000' is optional, but it makes for nicer zip
      // files if sizes are known in advance.
      ZipAddHandle(hz, _T("simple3.dat"), hread, 1000);
    ...
    CloseHandle(hread);  // the thread will close hwrite
  }
#endif
  ...
  HANDLE hfz =
      CreateFile(_T("\\simple3 - pwd is 'password'.zip"), GENERIC_WRITE, 0, 0,
  ...
  SetUnzipBaseDir(hz, _T("\\"));
  int i;
  FindZipItem(hz, _T("simple.jpg"), true, &i, &ze);
  //   - unzip to a file -
  ...
  while (zr == ZR_MORE) {
    ...
    if (zr == ZR_OK) bufsize = ze.unc_size - totsize;
...
void CreateFiles() {
  ...
  BYTE *bits;
  HBITMAP hbm =
      CreateDIBSection(sdc, &bi, DIB_RGB_COLORS, (void **)&bits, NULL, 0);
  ...
  ReleaseDC(0, sdc);
  //
  hf = CreateFile(_T("\\simple.bmp"), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
  ...
  //
  // Dummy file: text
  hf = CreateFile(_T("\\simple.txt"), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
                  FILE_ATTRIBUTE_NORMAL, NULL);
  const char *src =
      "Few things are more distressing\r\nto a well regulated mind\r\nthan to "
      "see a boy who ought to know better\r\ndisporting himself at improper "
      "moments.\r\n";
